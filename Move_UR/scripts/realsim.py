...
def send_multi_view_online_data(hand_img, scene_img, traj):
        url =  "http://10.184.17.53:4103/vla"
        assert isinstance(hand_img, np.ndarray) and isinstance(scene_img, np.ndarray)
        print(hand_img.shape, scene_img.shape)
        _, hand_encoded = cv2.imencode('.jpg', np.array(hand_img))
        _, scene_encoded = cv2.imencode('.jpg', np.array(scene_img))

        hand_bytes = io.BytesIO(hand_encoded.tobytes())
        scene_bytes = io.BytesIO(scene_encoded.tobytes())

        traj_str = ','.join([str(x) for x in traj])

        files = {
            "image_file": ("hand.jpg", hand_bytes, "image/jpeg"),
            "scene_file": ("scene.jpg", scene_bytes, "image/jpeg"),
        }
        data = {
            "traj_str": traj_str
        }
        ...
        if response.status_code == 200:
            ...
            return task_info["action"]
        else:
            return None

def send_single_view_online_data(action):
    action_bytes = io.BytesIO()
    np.save(action_bytes, action)
    action_bytes.seek(0)
    files = {
        "action_file": ("action.npy", action_bytes, "application/action"),
    }
...
class MoveClient:
    def __init__(self):
        rospy.init_node('realsim_client')
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            if self.ur_endeffector_position is not None :
                ...
                print("hereeee*************************************")
                # action =  send_multi_view_online_data(hand_img=hand_view, scene_img = img_color, traj=proprio_array)
                img_color = None
                action = send_single_view_online_data(img_color, proprio_array)
                if action is None:
                    continue
